...
def test_iphone15_search_relevance_isolation():
    ...
    Must EXCLUDE: iPhone 16, 14, 15 Pro, 15 Pro Max, iPad, MacBook, AirPods, Watch, Cases
    """
    ...
    wrong_variant_pro = DummyProduct(
        "Apple iPhone 15 Pro (128 GB) - Natural Titanium", "Apple", "Mobiles"
    )
    wrong_variant_promax = DummyProduct(
        "Apple iPhone 15 Pro Max (256 GB) - Black Titanium", "Apple", "Mobiles"
    )
...
def test_samsung_galaxy_s25_ultra_relevance_isolation():
    ...
    Must EXCLUDE: Samsung Galaxy S25, S25+, S24 Ultra, Galaxy A15, Case
    """
    valid_ultra = DummyProduct(
        "Samsung Galaxy S25 Ultra 5G (512 GB) - Titanium Gray", "Samsung", "Mobiles"
    )
...
def test_macbook_air_m4_relevance_isolation():
    ...
    """
    valid_m4_air = DummyProduct(
        "Apple MacBook Air M4 Chip (16GB RAM, 512GB SSD)", "Apple", "Laptops"
    )
    wrong_pro = DummyProduct(
        "Apple MacBook Pro M4 Chip (18GB RAM, 512GB SSD)", "Apple", "Laptops"
    )
    wrong_m3 = DummyProduct(
        "Apple MacBook Air M3 Chip (8GB RAM, 256GB SSD)", "Apple", "Laptops"
    )
...
def test_airpods_pro_relevance_isolation():
    ...
    """
    valid_pro = DummyProduct(
        "Apple AirPods Pro (2nd Generation) with MagSafe Case", "Apple", "Headphones"
    )
    wrong_gen = DummyProduct(
        "Apple AirPods (3rd Generation) with Lightning Charging Case", "Apple", "Headphones"
    )
    wrong_max = DummyProduct(
        "Apple AirPods Max Wireless Over-Ear Headphones", "Apple", "Headphones"
    )
    wrong_case = DummyProduct(
        "Silicone Protective Case Cover for Apple AirPods Pro", "Spigen", "Accessories"
    )
...
def test_ps5_relevance_isolation():
    ...
    wrong_accessory = DummyProduct(
        "Sony DualSense Wireless Controller for PlayStation 5", "Sony", "Accessories"
    )
